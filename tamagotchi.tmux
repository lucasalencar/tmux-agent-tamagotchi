...
# The hooks the plugin needs, as `<tmux hook>|<what bin/tama is asked to do>`.
#
...
# and `set-hook -ga` would otherwise stack another copy on each reload.
#
...
# the check below can look for it exactly, a user who moves their clone does not need
# to rewire anything, and there is no stale absolute path left in a hook pointing at a
...
wire_hook() { # <tmux hook> <tama command>
  local recipe="#{q:@tama_bin} $2"

  # tmux prints a hook array back requoted — single quotes come out double — so this
  # looks for the part of the recipe that no requoting touches rather than the whole
  # line. There is nothing in it for tmux to requote, and naming the plugin's own
  # option and the command together is specific enough that no other hook on the
  # event could collide with it.
  if tmux_run show-options -g "$1" 2>/dev/null | grep -qF -- "$recipe"; then
    return 0
  fi

  # A failed write is not worth failing a config reload over.
  tmux_run set-hook -ga "$1" "run-shell -b '$recipe'" >/dev/null 2>&1 || true
  return 0
}
