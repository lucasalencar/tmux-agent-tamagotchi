...
five seconds; leaving idle cancels the timer, and duplicate idle events do not postpone it.
A live delegated session suppresses completion eligibility even when every root is idle, and a
delegated session that starts during the five-second delay cancels the pending notification. Its
